...
type Game = { id: string; status: "scheduled" | "live" | "final"; home_team_id: string; away_team_id: string; home_score: number; away_score: number; period: string | null; clock: string | null; scheduled_at: string; home_team?: Team | null; away_team?: Team | null; };
type Prop = { id: string; game_id: string | null; prop_type: "h2h_player" | "game_total_pim" | "next_team_to_score"; status: "open" | "locked" | "resolved" | "void"; points_reward: number; locks_at: string | null; metadata: any; };
type PropPick = { id: string; prop_id: string; selection: any; };

export default function LiveView({ games, props, myPicks }: { games: Game[]; props: Prop[]; myPicks: PropPick[]; }) {
    ...
    <div className="space-y-6">
      ...
      {props.length > 0 && (
        <section>
          ...
          <div className="space-y-2">
            {props.map((p) => (
              <PropCard key={p.id} prop={p} existingPick={myPicks.find((pick) => pick.prop_id === p.id)} />
            ))}
          </div>
        </section>
      )}
...
function SectionHeader({ title, count, hot }: { title: string; count: number; hot?: boolean; }) {
  return (
    <div className="mb-3 flex items-baseline justify-between">
      <h2 className={cn("font-display text-xl font-black tracking-tight", hot && "text-live")}>{title}</h2>
      ...
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest">
        {isLive ? (
          ...
          <span className="text-ink-500">
            {new Date(game.scheduled_at).toLocaleString([], { weekday: "short", hour: "numeric", minute: "2-digit" })}
          </span>
        )}
      </div>
...
function TeamRow({ team, score, highlight }: { team: Team | null | undefined; score: number; highlight: boolean; }) {
  if (!team) return <div />;
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        {team.logo_url && (<img src={team.logo_url} alt="" className="h-6 w-6 object-contain" />)}
        <span className={cn("font-display text-[15px] font-bold", team.is_eliminated && "text-ink-500 line-through")} style={!team.is_eliminated ? { color: team.primary_color ?? undefined } : undefined}>
          {team.short_name}
        </span>
      </div>
      <span className={cn("font-display text-2xl font-black tabular-nums", highlight ? "text-ink-100" : "text-ink-400")}>{score}</span>
    </div>
  );
}
...
function PropCard({ prop, existingPick }: { prop: Prop; existingPick?: PropPick }) {
  ...
  const label = { h2h_player: "Grudge Match", game_total_pim: "Penalty Total", next_team_to_score: "Next Goal" }[prop.prop_type];
  const gameLabel = prop.metadata?.game_label ?? "";
    ...
    <motion.div layout className="overflow-hidden rounded-2xl border border-ink-700 bg-ink-850 shadow-card">
      <div className="px-4 pt-3 pb-2">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-md bg-brand/10 px-2 py-0.5 text-[10px] font-black text-brand">+{prop.points_reward} PTS</span>
        </div>
        {gameLabel && (<div className="mt-0.5 text-[11px] text-ink-500">{gameLabel}</div>)}
      </div>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              disabled={locked}
              className={cn("flex flex-col items-start bg-ink-850 px-4 py-4 text-left transition", picked && "bg-brand/10 ring-1 ring-inset ring-brand", !locked && !picked && "active:bg-ink-800")}
            >
              <span className="text-[10px] font-bold uppercase tracking-wider text-ink-500">{opt.subtitle}</span>
              <span className="mt-1 font-display text-[15px] font-bold">{opt.label}</span>
              {picked && (<span className="mt-1 text-[10px] font-bold text-brand">YOUR PICK</span>)}
            </button>
          );
        })}
      </div>
...
function getPropOptions(prop: Prop): { value: string; label: string; subtitle: string }[] {
  switch (prop.prop_type) {
    case "h2h_player":
      return [
        { value: "a", label: prop.metadata?.player_a_name ?? "Player A", subtitle: prop.metadata?.player_a_team ?? "OVER" },
        { value: "b", label: prop.metadata?.player_b_name ?? "Player B", subtitle: prop.metadata?.player_b_team ?? "VS" },
      ];
    case "game_total_pim":
      return [
