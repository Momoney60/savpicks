...
export default function BracketView({
  ...
  function getPreviewStreak(teamId: string, currentRound: number) {
    ...
    for (let r = currentRound - 1; r >= 1; r--) {
      if (pickByTeamByRound[r]?.has(teamId)) streak++;
      else break;
    }
    return streak;
  }
        ...
        {[1, 2, 3, 4].map((r) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex-1 rounded-lg py-2 text-[12px] font-bold tracking-wider transition",
                round === r ? "bg-brand text-ink-900" : "text-ink-400"
              )}
            >
      ...
      {roundSeries.length === 0 ? (
        <div className="rounded-2xl border border-ink-700 bg-ink-850 p-8 text-center">
          ...
          <p className="mt-4 font-display text-lg font-bold">
            Round {round} not set yet
          </p>
          <p className="mt-1 text-sm text-ink-400">
            The commissioner will load matchups once the previous round resolves.
          </p>
        </div>
      ) : (
...
function SeriesCard({
    ...
    return (
      <div className="rounded-xl border border-ink-700 bg-ink-850 p-5 opacity-50">
        <div className="text-xs text-ink-400">
          {series.conference ?? "Cup Final"} · TBD
        </div>
        <div className="mt-2 font-display text-base text-ink-300">
          Matchup pending prior round
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
        ...
        {[teamA, teamB].map((team) => {
          ...
          return (
            <button
              ...
              style={picked && !locked ? { boxShadow: `inset 4px 0 0 0 ${team.primary_color}` } : undefined}
            >
              ...
              {picked && !locked && (
                <span className="absolute top-2 right-2 text-[10px] font-bold text-brand">
                  YOUR PICK
                </span>
              )}
            </button>
      ...
      {/* Footer: lock time or awarded points */}
      {existingPick?.awarded_points ? (
        <div className="border-t border-ink-700 bg-brand/5 px-4 py-2.5 text-center">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-brand">
            +{existingPick.awarded_points} pts
            {existingPick.streak_length && existingPick.streak_length > 1 && (
              <span className="ml-1">· {existingPick.streak_length}× streak</span>
            )}
          </span>
        </div>
